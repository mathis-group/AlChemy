...
#define	  BIG     2000		/* size of local arrays */
...
PUBLIC void lambda_status (FILE * fp);

PRIVATE int locate (char *name, char *type);
...
PRIVATE void print_type (int dummy, int *count);
...
initialize_lambda (parmsLambda * Params)
{
  ...
  for (i = 0; i <= Interp->parms->symbol_table_size; i++)
    {
      ...
      Interp->table[i].type   = (char *) space (sizeof (char) * (Interp->parms->type_length + 2));
      Interp->table[i].key = 0;
      Interp->table[i].next = 0;
    }

  Interp->stack = (pair *) space (sizeof (pair) * (Interp->parms->stack_size + 1));
  Interp->type = (int *) space (sizeof (int) * (Interp->parms->symbol_table_size + 1));
  ...
  Interp->table[locate (" pred      ", "\0")].key = 1;
  Interp->table[locate (" zero      ", "\0")].key = 2;
  Interp->table[locate (" succ      ", "\0")].key = 3;
  Interp->table[locate (" null      ", "\0")].key = 4;
  Interp->table[locate (" add       ", "\0")].key = 5;
  Interp->table[locate (" sub       ", "\0")].key = 6;
  Interp->table[locate (" mult      ", "\0")].key = 7;
  Interp->table[locate (" div       ", "\0")].key = 8;
  Interp->table[locate (" iota      ", "\0")].key = 15;
  Interp->table[locate (" show      ", "\0")].key = 16;
  Interp->table[locate (" more      ", "\0")].key = 17;
  Interp->table[locate (" not       ", "\0")].key = 20;
  Interp->table[locate (" true      ", "\0")].key = 21;
  Interp->table[locate (" false     ", "\0")].key = 22;
  Interp->table[locate (" and       ", "\0")].key = 23;
  Interp->table[locate (" or        ", "\0")].key = 24;
  Interp->table[locate (" map       ", "\0")].key = 25;
  Interp->table[locate (" append    ", "\0")].key = 26;
...
get_token (int *n, float *x)
{
  ...
  int location;
  char name[SMALL];
  char type[SMALL];  /* basis types aren't that big */
  float place;
  ...
  *n = 0;
  ...
  if (strchr (L->letters, L->peek) != NULL)
	...
	{
	  name[i++] = L->peek; /* no overflow check... */
	  L->peek = str_getc (L->input_expression);
	}
      for (j = i; j <= L->parms->name_length; name[j++] = ' ');
      name[L->parms->name_length + 1] = '\0';
	...
	{			/* TYPE TOKEN */
	  ...
	  while (L->peek != ']')
	    {
	      type[i++] = L->peek; /* no overflow check... */
	      L->peek = str_getc (L->input_expression);
	    }
	  type[i] = '\0';
	  L->peek = str_getc (L->input_expression);
  ...
  while (more)
	...
	default:		/* ---- renaming prefix ---- */
	  ...
	  if (L->heap[point].code < 0)
	    ...
	    {
	      err ("print_expression: Wrong Expression!\n");
	      more = FALSE;
	    }
	  break;
...
PRIVATE void
print_type (int dummy, int *count)
{
  register int i;
  
  if (L->table[dummy].type[0]) 
    {
      print_char ('[', count);
      for (i = 0; i < strlen (L->table[dummy].type); i++)
	print_char (L->table[dummy].type[i], count);
      print_char (']', count);
    }
}
...
beta3 (void)
{
  ...
  /* 
     generate a new name and remember the original type 
     through the original symbol table entry 
  */

  L->sys_var--;
  if (-L->sys_var >= L->parms->symbol_table_size)
    {
      L->error.symbol_table_overflow = TRUE;
      L->error.symbol_table_overflow_hits++;
      err ("Too many new names.\n");
    }
  sym = L->node[L->n3].op1;
  if (sym < 0) sym = L->type[-sym];
  L->type[-L->sys_var] = sym;
  ...
  if (done)
    {
      ...
      if (answer)
	{
	  L->node[L->n1].op1 = locate (" TRUE      ", "\0");
	  L->node[L->n1].u.op2 = 21;
	}
      else
	{
	  L->node[L->n1].op1 = locate (" FALSE     ", "\0");
  ...
  while (more)
	...
	default:		/* ---- renaming prefix ---- */
	  ...
	  if (L->heap[point].code < 0)
	    ...
	    {
	      err ("alpha_standardize: Wrong Expression!\n");
	      more = FALSE;
	    }
	  break;
...
err (char *message)
{
  ...
  if (L->parms->error_fp)
    {
      fprintf (L->parms->error_fp, "error #%d at expression ", L->error_number);
...
/* Compile with -DLAMBDA_TEST to make an executable for use in testing */
...
main (int argc, char **argv)
{
  ...
  Parameters->type_length = 100; /* max length of basis types */
  ...
  printf ("test suite (0) or interactive mode (1)\n");
  scanf ("%d", &mode);
  ...
  if (mode)
    ...
    {
      fp = fopen ("lambda.tst", "r");
      if (fp == NULL)
	{
	  printf ("no file lambda.tst\n");
	  exit (0);
	}
