...
def run_L0_seed():
    ...
    if os.path.exists("L0_seeds.csv"):
        old_params_df = pd.read_csv("L0_seeds.csv", index_col=0)
        combined_params_df = pd.concat([old_params_df, seed_params_df], ignore_index=True)
        combined_params_df.to_csv("L0_seeds.csv")
    else:
        seed_params_df.to_csv("L0_seeds.csv")
